...
namespace Milou.Deployer.Web.Marten
{
            ...
            AssignedAgentsInPoolsQueryResult>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var pools = await session.Query<AgentPoolData>().ToListAsync();
            var agents = await session.Query<AgentData>().ToListAsync();

            return Map(agentPoolAssignmentData, pools, agents);
        }
